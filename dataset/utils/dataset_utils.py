...
def split_data(X, y, balanced_attribute=None):
    # Split dataset
    ...
    for i in range(len(y)):
        if balanced_attribute is not None:
            X_group_list = []
            y_group_list = []
            unique_ba_list = set(balanced_attribute)
            for u_ba in unique_ba_list:
                ba_index_list = [i for i, x in enumerate(dataset_label) if x == uba]
                X_group_list.append(X[i] for i in ba_index_list)
                y_group_list.append(y[i] for i in ba_index_list)
            X_train, X_test, y_train, y_test = [], [], [], []
            for gid in range(len(unique_ba_list)):
                X_train_i, X_test_i, y_train_i, y_test_i = train_test_split(
                    X_group_list[gid][i], y_group_list[gid][i], train_size=train_size, shuffle=True, stratify=y[i])
