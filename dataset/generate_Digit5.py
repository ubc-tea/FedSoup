...
def digit5_dataset_read(base_path, domain):
    ...
    train_dataset = Digit5Dataset(
        data=train_image, labels=train_label, transform=transform
    )
    train_loader = DataLoader(
        dataset=train_dataset, batch_size=len(train_dataset), shuffle=True
    )
    test_dataset = Digit5Dataset(
        data=test_image, labels=test_label, transform=transform
    )
...
random.seed(1)
np.random.seed(1)
ROOT_DIR = "/home/chenmh/projects/rrg-timsbc/chenmh/cmh_proj/dataset/Digit5/"
data_path = ROOT_DIR
dir_path = ROOT_DIR
