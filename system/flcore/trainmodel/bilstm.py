...
class BiLSTM_TextClassification(nn.Module):
    ...
    def attention_forward(self, lstm_output, state, seq_lens):
        ...
        for i, seq_len in enumerate(seq_lens):
            ...
            new_hidden = torch.matmul(
                soft_attn_weights.unsqueeze(0), lstm_output[i, :seq_len, :]
            )
    ...
    def forward(self, x):
        ...
        h_0 = torch.zeros((self.num_layers * 2, batch_size, self.hidden_size)).to(
            device="cuda"
        )
        c_0 = torch.zeros((self.num_layers * 2, batch_size, self.hidden_size)).to(
            device="cuda"
        )

        input_seq = input_seq.permute(1, 0, 2)
        output, (final_hidden_state, final_cell_state) = self.lstm_layer(
            input_seq, (h_0, c_0)
        )
        ...
        state = torch.cat(
            [
                output[i, seq_len - 1, :].unsqueeze(0)
                for i, seq_len in enumerate(seq_lens)
            ],
            dim=0,
        )
